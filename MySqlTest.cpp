...
class Property {
    ...
    double price = 0.0;
    string lastrev;
    string laundry, parking; //laundary -> Y,n; Parking -> S(street),G(Garage),N(none)
...
void searchProperty(Users& u) {
    ...
    cout << "Welcome user:\t" << u.getData('U') << endl;
    cin.ignore();
    cout << "Enter the area:\t";
    getline(cin,ar1);
    ...
    while (res->next()) {
        if (res->getInt("Prop_ID") == c) {
            cout << res->getInt("Prop_ID") << "\t" << res->getInt("size") << "\t" << res->getDouble("price") << "\t" << res->getInt("b") << "\t" << res->getInt("br") << "\t\t" << res->getString("laundary") << "\t" << res->getString("parking") <<endl;
        }
    }
}
